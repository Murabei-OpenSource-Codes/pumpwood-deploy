...
class BigdataCorpCrawler:
    ...
    def __init__(self, db_username, db_password, micro_username,
                 micro_password, bdc_username, bdc_password, bucket_name,
        ...
        Args:
            ...
            bdc_username (str): Usename from big data corp APIs.
            bdc_password (str): Password from big data corp APIs.
        ...
        self._bdc_username = base64.b64encode(
            bdc_username.encode()).decode()
        self._bdc_password = base64.b64encode(
            bdc_password.encode()).decode()
    ...
    def create_deployment_file(self):
        ...
        secrets_text_formated = secrets_text.format(
            ...
            bdc_username=self._bdc_username,
            bdc_password=self._bdc_password,
        ...
        list_return = [{
                'type': 'services',
                'name': 'crawler__bigdata_corp__services',
                'content': services_text_formated, 'sleep': 0},
            {
                'type': 'secrets',
                'name': 'crawler__bigdata_corp__secrets',
                'content': secrets_text_formated, 'sleep': 5},
            {
                'type': 'volume', 'name': 'crawler__bigdata_corp__volume',
                'content': volume_postgres_text_formated, 'sleep': 10},
            {
                'type': 'deploy',
                'name': 'crawler__bigdata_corp__postgres',
                'content': deployment_postgres_text, 'sleep': 0},
            {
                'type': 'deploy',
                'name': 'crawler__bigdata_corp__queue_manager',
                'content': deployment_queue_manager_text_formated, 'sleep': 0},
            {
                'type': 'deploy',
                'name': 'crawler__bigdata_corp__puller',
                'content': deployment_puller_text_formated, 'sleep': 0},
            {
                'type': 'deploy',
                'name': 'crawler__bigdata_corp__pusher',
        ...
        if self.firewall_ips and self.postgres_public_ip:
            ...
            list_return.append({
                'type': 'services',
                'name': 'crawler__bigdata_corp__services_loadbalancer',
